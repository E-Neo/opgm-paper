...
\textcolor{red}{Moreover, we design an compact layout that is able to write the compressed data sequentially.}
...
\textcolor{red}{In order to reduce the I/O cost for storing SuperRows, matched vertices from the \textsc{NeighborIter} will be appended to the SuperRow file. However, leaves with the same color in a star may belong to different neighbor equivalence classes if the connections to the root are different. For example, $u_2$ and $u_3$ will be different if the edge $u_2 \rightarrow$in Figure~\ref{img:running_example}}
