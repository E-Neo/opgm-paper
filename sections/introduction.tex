...
The conventional way to store graphs is the compressed sparse row (CSC) and compressed sparse column (CSR) approach which stores in/out-edges \emph{separately}~\cite{DBLP:conf/sc/PearceGA10,DBLP:conf/osdi/KyrolaBG12}.
In contrast, SeqStar stores the graph data by storing the in/out-edges \emph{together} with the neighbors of each vertex.
By doing so, SeqStar can avoid random disk seeks and performance graph matching in a sequential disk scan.
