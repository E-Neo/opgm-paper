...
Like other systems, there are two fundamental components in SeqStar:
the graph storage engine and the graph matching engine,
and challenges lie in these two parts.

To handle the real-world property graph efficiently, we propose the \emph{vertex-centric storage model}.
This can eliminate the random access problem in CSC and CSR which store in/out-edges separately.

To reduce memory usage, we make the following contributions in SeqStar's graph matching engine:
(1) a novel star decomposition algorithm that keeps all the useful filtering information on stars,
