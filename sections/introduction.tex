...
The storage engine in SeqStar uses the \emph{vertex-centric storage model} which stores the information related to a specific vertex together. Such a model can eliminate the random access problem in CSC and CSR which store in/out-edges separately. %这里加上csc和csr的全称，并加上引用
