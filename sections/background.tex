...
\end{definition}
There are two graphs in a property graph isomorphism problem:
one is the large data graph, the other is the (much smaller) pattern graph.
The problem is to find all subgraphs of the data graph that are isomorphic to the pattern.
...
However, the WHERE clause is \emph{general}, users of a real-world graph database usually provide their self-defined searching condition $\psi$ to filter out unnecessary matchings not only symmetry-breaking conditions.
