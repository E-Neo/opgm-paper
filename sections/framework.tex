...
For each neighbor equivalence class, the matched vertices are stored together as a \emph{image set}.
For example $\{2, 3, 5, 6\}$ in $T_1$ is the image set of $u_2$ and $u_3$.
Image set is the basic structure to hold the intermediate results.
...
Finally, SeqStar joins on these SuperRows to obtain the final results (\S\ref{sec:match_join}).
